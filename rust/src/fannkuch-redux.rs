...
struct BenchmarkResult {
    ...
    peak_memory: u64, // Add a field for peak memory usage
}
...
fn fannkuch_benchmarked(n: i32) -> (i32, i32, BenchmarkResult) {
    let start = time::Instant::now(); // Start timing
    ...
    let duration = start.elapsed(); // End timing
     ...
     let peak_memory = process.memory_info().unwrap().rss();
 
     (checksum, maxflips, BenchmarkResult { execution_time: duration, peak_memory: peak_memory })
    }
...
fn main() {
    ...
    let (checksum, maxflips, benchmark) = fannkuch_benchmarked(n);
    // println!("{}\nPfannkuchen({}) = {}", checksum, n, maxflips);
    println!("Time elapsed: {:?}", benchmark.execution_time); // Print elapsed time
    println!("Peak memory usage: {} bytes", benchmark.peak_memory); // Print peak memory usage
